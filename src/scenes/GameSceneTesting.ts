...
export function installTestHooks(scene: any): void {
  ...
  scene.detachTestBridge = installTestBridge(scene.testMode, {
    ...
    targets: () => ({
      bay: { ...WORLD.bay },
      exit: { ...WORLD.exit },
      cores: WORLD.cores.map((item) => ({ ...item.position })),
      switches: WORLD.switches.map((item) => ({ ...item.position })),
      coolPads: WORLD.coolPads.map((item) => ({ ...item })),
      lockers: WORLD.lockers.map((item) => ({ ...item.position })),
      repairs: WORLD.repairs.map((item) => ({ ...item.position })),
      doors: Object.fromEntries(WORLD.doors.map((item) => [item.id, { ...item.position }])),
    }),
  });
}
...
export function pointOf(scene: any, object: Phaser.GameObjects.Components.Transform): Point {
