...
export class VoiceOverInput {
  ...
  attach(root: HTMLElement): () => void {
    const onClick = (event: Event) => {
      ...
      if (command === "forward") this.hub.emit({ type: "move", amount: 1, durationMs: 510 });
      else if (command === "fast") this.hub.emit({ type: "move", amount: 1.6, durationMs: 620 });
      else if (command === "back") this.hub.emit({ type: "move", amount: -0.6, durationMs: 500 });
