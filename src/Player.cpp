...
void Player::attack(std::vector<std::unique_ptr<Enemy>>& enemies)
{
    ...
    {
        m_weapons[m_currentWeaponIndex]->attack();

        if (enemies.front()->isDead()) return; // Не атакуем уже мертвого врага

        Enemy& target = *enemies.front();

        int damage = m_weapons[m_currentWeaponIndex]->getDamage();
        target.takeDamage(damage);

        if (m_weapons[m_currentWeaponIndex]->isStunWeapon())
        {
            target.stun();
        }
    }
