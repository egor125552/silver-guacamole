...
void SoundEngine::activateDirectionalSonar(int numpadKey) {
    ...
    for (const auto& wall : walls) {
        ...
        if (intersection.x != -1) {
            float dist_sq = (intersection.x - playerPos.x) * (intersection.x - playerPos.y) * (intersection.y - playerPos.y);
        ...
        if (npc_to_update->isAlive) {
            ...
            if (npc_to_update->state != AIState::COMBAT) {
                for (const auto& other_npc : npcs) {
                    if (other_npc->isAlive && other_npc.get() != npc_to_update.get() && other_npc->state == AIState::COMBAT) {
