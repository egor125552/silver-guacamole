...
for(let run=0;run<60;run++){
 ...
 for(const stage of CampaignStages){ok(!g.state.walls.some(w=>pointInRect(stage.target,w)),`objective ${stage.id} inside wall on run ${run}`);ok(campaignLegIsFree(g,previous,stage.target),`mandatory route to ${stage.id} blocked on run ${run}`);previous=stage.target;}
...
console.log('Campaign randomized safety passed: 60 generated worlds with walkable objective routes');
