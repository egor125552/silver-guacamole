...
import {rectIntersects} from "../../core/math.js";
...
function clearCampaignCorridor(walls,from,to,halfWidth=3){
  const horizontal={x:Math.min(from.x,to.x)-halfWidth,z:from.z-halfWidth,w:Math.abs(to.x-from.x)+halfWidth*2,h:halfWidth*2};
  const vertical={x:to.x-halfWidth,z:Math.min(from.z,to.z)-halfWidth,w:halfWidth*2,h:Math.abs(to.z-from.z)+halfWidth*2};
  return walls.filter(w=>!rectIntersects(w,horizontal)&&!rectIntersects(w,vertical));
}
...
export default{id:"campaign-state",install(game){
  game.campaignStage=()=>CampaignStages[game.state.campaign?.stage??0]||null;
  game.campaignObjectivePosition=()=>{const c=game.state.campaign;if(!c)return null;if(c.objectiveEnemyId!=null){const e=game.state.enemies.find(x=>x.id===c.objectiveEnemyId&&x.alive);if(e)return e.position;}return game.campaignStage()?.target||null;};
  game.campaignDescribe=()=>{const c=game.state.campaign,s=game.campaignStage();if(!c||!s)return c?.completed?"Побег завершён.":"";let extra="";if(c.holdUntil>game.time)extra=` Осталось удерживать позицию ${Math.ceil(c.holdUntil-game.time)} секунд.`;if(s.id==="exit"&&game.time<c.minimumVictoryTime)extra=` Ворота заблокированы. До открытия ${Math.ceil(c.minimumVictoryTime-game.time)} секунд. Держитесь.`;const arsenal=game.state.player?.unlockedWeapons&&game.weaponName?` Доступное оружие: ${[...game.state.player.unlockedWeapons].map(game.weaponName).join(", ")}.`:"";return `Этап ${c.stage+1} из ${CampaignStages.length}. Сложность: ${s.difficulty}. ${s.text} Подсказка: ${s.hint}${arsenal}${extra}`;};
  return game.on("reset",()=>{let previous={x:0,z:0};for(const stage of CampaignStages){game.state.walls=clearAround(game.state.walls,stage.target,9);game.state.walls=clearCampaignCorridor(game.state.walls,previous,stage.target,3);previous=stage.target;}game.state.campaign={active:true,stage:0,stageSince:0,objectiveEnemyId:null,holdUntil:0,nextWaveAt:0,wave:0,completed:false,minimumVictoryTime:20*60};prepareOpeningPopulation(game);});
}};
